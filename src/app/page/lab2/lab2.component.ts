...
import { ListProducts } from './../../interface/interface-list';
@Component({
  selector: 'app-lab2',
...
export class Lab2Component implements OnInit {
  ...
  ngOnInit(): void {
    this.list_Product_Search=this.ProductList;
  }

  list_Product_Search:ListProducts[]=[];
  search = '';
  filterValue:string= this.search;
  filter(){
    this.ProductList=this.list_Product_Search.filter(p=>p.productName.includes(this.filterValue))
    }
}
